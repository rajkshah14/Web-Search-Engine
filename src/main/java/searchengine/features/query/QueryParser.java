...
import searchengine.features.dictionary.DictionaryManager;
import searchengine.features.dictionary.IDict;
...
public class QueryParser {
    ...
    private List<String> unstemmedTerms;
    ...
    public QueryParser(String query, String lang)
    {
        ...
        for (int i = 0; i < terms.length; i++) {
            ...
            }else if(terms[i].startsWith("\"") && terms[i].endsWith("\""))
            {
                ...
                unstemmedTerms.add(terms[i].substring(1,terms[i].length()-1)); //TODO should they also be stemmed?
            }else if(terms[i].startsWith("~")){
                synonyms.add(terms[i].substring(1));
                unstemmedTerms.add(terms[i].substring(1));
            }else {
                this.unstemmedTerms.add(terms[i]);
    ...
    public List<String> getAllUnstemmedTerms(){
